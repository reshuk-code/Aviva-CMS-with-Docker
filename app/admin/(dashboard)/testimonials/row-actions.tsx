...
export function TestimonialRowActions({
  ...
  testimonial: { id: string; name: string; status: ContentStatus };
  canUpdate: boolean;
  canDelete: boolean;
  canPublish: boolean;
  canCreate: boolean;
  ...
  function setStatus(status: ContentStatus) {
    run(() => setTestimonialStatusAction(testimonial.id, status));
  }
    ...
    <div className="flex items-center justify-end gap-1">
      {canUpdate ? (
        <Button variant="ghost" size="sm" asChild>
          <Link href={`/admin/testimonials/${testimonial.id}`}>
            ...
            <span className="sr-only">Edit the review from {testimonial.name}</span>
          </Link>
        </Button>
      ) : null}
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <Button variant="ghost" size="sm" disabled={pending}>
            ...
            <span className="sr-only">More actions for {testimonial.name}</span>
          </Button>
        ...
        <DropdownMenu.Portal>
          ...
          >
            {canPublish && testimonial.status !== "published" ? (
            ...
            {canDelete && testimonial.status === "trash" ? (
              <DropdownMenu.Item className={itemClass} onSelect={() => setStatus("draft")}>
                <Undo2 className="size-4" />
                Restore from trash
              </DropdownMenu.Item>
            ...
            {canCreate ? (
              <DropdownMenu.Item
                className={itemClass}
                onSelect={() => run(() => duplicateTestimonialAction(testimonial.id))}
              >
      ...
      {canDelete && testimonial.status === "trash" ? (
        <ConfirmButton
          variant="ghost"
          size="sm"
          title={`Delete the review from "${testimonial.name}"?`}
          description="This removes the review permanently, including the record that it was ever given. This cannot be undone."
        ...
        >
          ...
          <span className="sr-only">
            Delete the review from {testimonial.name} permanently
          </span>
        </ConfirmButton>
      ) : null}
    </div>
