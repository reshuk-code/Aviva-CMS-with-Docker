...
/**
 * Row actions for an activity. No View/Preview entry, for the same reason as
 * posts and destinations: the CMS does not know which route the project serves
 * them at.
 */
export function ActivityRowActions({
  ...
  activity: { id: string; name: string; status: ContentStatus; usedByTours: number };
  canUpdate: boolean;
  canDelete: boolean;
  canPublish: boolean;
  canCreate: boolean;
  ...
  function setStatus(status: ContentStatus) {
    run(() => setActivityStatusAction(activity.id, status));
  }
    ...
    <div className="flex items-center justify-end gap-1">
      {canUpdate ? (
        <Button variant="ghost" size="sm" asChild>
          <Link href={`/admin/activities/${activity.id}`}>
            ...
            <span className="sr-only">Edit {activity.name}</span>
          </Link>
        </Button>
      ) : null}
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <Button variant="ghost" size="sm" disabled={pending}>
            ...
            <span className="sr-only">More actions for {activity.name}</span>
          </Button>
        ...
        <DropdownMenu.Portal>
          ...
          >
            ...
            {canDelete && activity.status === "trash" ? (
              <DropdownMenu.Item className={itemClass} onSelect={() => setStatus("draft")}>
                <Undo2 className="size-4" />
                Restore from trash
              </DropdownMenu.Item>
            ...
            {canCreate ? (
              <DropdownMenu.Item
                className={itemClass}
                onSelect={() => run(() => duplicateActivityAction(activity.id))}
              >
      ...
      {canDelete && activity.status === "trash" ? (
        <ConfirmButton
          ...
          description={
            activity.usedByTours > 0
              ? `${activity.usedByTours} tour package${
                  activity.usedByTours === 1 ? " still tags" : "s still tag"
                } this activity. Deleting it leaves those tags pointing at nothing — they will simply stop appearing. This cannot be undone.`
              : "This removes the activity permanently. No tour packages currently tag it. This cannot be undone."
          }
        ...
        >
          ...
          <span className="sr-only">Delete {activity.name} permanently</span>
        </ConfirmButton>
      ) : null}
    </div>
  );
