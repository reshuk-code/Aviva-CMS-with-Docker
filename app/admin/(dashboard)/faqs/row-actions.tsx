...
export function FaqRowActions({
  ...
  faq: { id: string; question: string; status: ContentStatus };
  canUpdate: boolean;
  canDelete: boolean;
  canPublish: boolean;
  canCreate: boolean;
  ...
  function setStatus(status: ContentStatus) {
    run(() => setFaqStatusAction(faq.id, status));
  }
    ...
    <div className="flex items-center justify-end gap-1">
      {canUpdate ? (
        <Button variant="ghost" size="sm" asChild>
          <Link href={`/admin/faqs/${faq.id}`}>
            ...
            <span className="sr-only">Edit {faq.question}</span>
          </Link>
        </Button>
      ) : null}
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <Button variant="ghost" size="sm" disabled={pending}>
            ...
            <span className="sr-only">More actions for {faq.question}</span>
          </Button>
        ...
        <DropdownMenu.Portal>
          ...
          >
            {canPublish && faq.status !== "published" ? (
            ...
            {canCreate ? (
              <DropdownMenu.Item
                className={itemClass}
                onSelect={() => run(() => duplicateFaqAction(faq.id))}
              >
      ...
      {canDelete && faq.status === "trash" ? (
        <ConfirmButton
          ...
          description="This removes the question and its answer permanently. Any page rendering it will simply stop showing it. This cannot be undone."
        ...
        >
          ...
          <span className="sr-only">Delete {faq.question} permanently</span>
        </ConfirmButton>
      ) : null}
    </div>
  );
